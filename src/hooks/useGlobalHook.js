...
const GlobalHooksContext = createContext();

export function GlobalHooksProvider({ hooks, children }) {
  let globalHooks = {};
  const parentContext = useContext(GlobalHooksContext);
...
export function useGlobalHook(name) {
  const context = useContext(GlobalHooksContext);
...
export function withGlobalHooks(component, hooks) {
  ...
  const withGlobalHOC = (props) => {
    ...
    return createElement(component, { ...props, ...stores });
  };
